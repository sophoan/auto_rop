...
class ROPProcessor():
    ...
    def calculate_rop(self, product, use_new_cursor=False):
        ...
        lead_time_forecaster = MovingAverageLeadTimeForecaster()
        lead_time = lead_time_forecaster.get(product)

        print("Lead time: {0}".format(lead_time))


        sale_forecaster = MovingAverageSaleForecaster()
        sale = sale_forecaster.get_for_next_ndays(product, lead_time)
        ...
        for orderpoint in product.orderpoint_ids:
            ...
            self.save_rop_result(product.env['auto_rop.rop'],{
            ...
            self.update_reorder_rule(orderpoint, {
                'product_min_qty': new_min_qty,
                'product_max_qty': new_max_qty,
                'lead_days': new_lead_time
            })
            if use_new_cursor:
                product._cr.commit()
...
class ROPForecaster():
    def get_forecasted_rop(self, product, days):
        pass
...
class MovingAverageSaleForecaster(SaleForecaster):
    def get_for_next_ndays(self, product, days):
        if days == 0:
            return 0
        date_1 = fields.Datetime.to_string((fields.Datetime.from_string(fields.Datetime.now()) - timedelta(days=days)))
        date_2 = fields.Datetime.to_string((fields.Datetime.from_string(fields.Datetime.now()) - timedelta(days=365)))
        date_3 = fields.Datetime.to_string((fields.Datetime.from_string(fields.Datetime.now()) - timedelta(days=365-days)))
